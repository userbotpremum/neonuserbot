...
CmdHelp('filter').add_command(
    ...
    'filter', '<filterlənəcək söz> <cavablanacaq mətn> vəya bir mesajı .filter <filterləcənək söz>', 'Filter əlavə edər. Nə zaman əlavə etdiyinz söz/cümla yazılarsa bot cavab verər.', '.filter "salam" "aleykum"'
).add_command(
    'stop', '<filtre>', 'Seçilen filteri dayandırar.'
).add_command(
    'ümumifilter | .genelfilter', '<filterlənəcək söz> <cavablanacaq mətn> vəya bir mesajı .genelfilter <filtrelənəcək söz>', 'Ümumi filtrə əlavə edəd. Bütün qruplarda işləyər.'
).add_command(
    'ümumistop | .genelstop', '<filtre>', 'Seçilən ümumi filteri dayandırar.'
).add()
