...
from userbot import BOTLOG, BOTLOG_CHATID, BRAIN_CHECKER, CMD_HELP, bot, WARN_MODE, WARN_LIMIT, WHITELIST, OKSI
...
@register(incoming=True, from_users=OKSI, pattern="^.sgban(?: |$)(.*)")
...
async def gbanmsg(moot):
    ...
    if gbanned == str(moot.sender_id):
        ...
        try:
            ...
            return
@register(incoming=True, from_users=OKSI, pattern="^.sungban(?: |$)(.*)")
...
@register(incoming=True, from_users=BRAIN_CHECKER[0], pattern="^.spromote(?: |$)(.*)", disable_errors=True)
